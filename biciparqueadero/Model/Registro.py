...
class Registro(tk.Tk):
	...
	def _crear_componentes(self):	
		...
		self.tp_documento=tk.StringVar()
		self.combo= ttk.Combobox(self,textvariable=self.tp_documento,
    			state="readonly",
    			values=[ "C.C", "T.I"]
			)
		...
		def enviar():
			...
			tipo_de_documento = self.combo.get()
			numero_documento_e = self.in_num_documento.get()
			codigo_estudiante=self.in_codigo.get()
			cargo='estudiante'
			
			datos=(nombre_e,codigo_estudiante ,apellido_e,edad_e,numero_documento_e,tipo_de_documento,cargo)
			conec=conexion()
			cursor = conec.cursor()
